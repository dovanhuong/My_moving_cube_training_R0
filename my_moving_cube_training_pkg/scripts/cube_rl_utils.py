...
class CubeRLUtils(object):
    ...
    def check_all_sensors_ready(self):
        ...
        while self.disk_joints_data is None and not rospy.is_shutdown():
            try:
                self.disk_joints_data = rospy.wait_for_message("/my_moving_cube/joint_states", JointState, timeout=1.0)
                rospy.loginfo("Current my_moving_cube/joint_states READY=>" + str(self.disk_joints_data))
        ...
        while self.disk_joints_data is None and not rospy.is_shutdown():
            try:
                self.cube_odom_data = rospy.wait_for_message("/my_moving_cube/odom", Odometry, timeout=1.0)
                rospy.loginfo("Current /my_moving_cube/odom READY=>" + str(self.cube_odom_data))
